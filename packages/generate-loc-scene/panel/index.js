...
async function sendRequestGenerate(mainFolderUUID, dataFolderUUID, languageList, fromLanguage, options) {
    ...
    Editor.Ipc.sendToMain('generate-loc-scene:generate', {mainFolderInfo, dataFolderInfo, languageList, isOverride: true, fromLanguage, options});
}
...
Editor.Panel.extend({
  ...
  template: `
    ...
    <br /><br />
    <ui-checkbox id="nodeTransform" checked></ui-checkbox>
    <span id="label">Keep Node</span>
    <br /><br />
    <ui-checkbox id="labelOptions" checked></ui-checkbox>
    <span id="label">Keep Label</span>
    <hr />
  ...
  $: {
    ...
    keepNodeTransform: '#nodeTransform',
    keepLabelOptions: '#labelOptions',
  },
  ...
  async ready () {
    this.$btn.addEventListener('confirm', ()=>{
      ...
      sendRequestGenerate(this.$mainFolder.value, this.$dataFolder.value, languageList, this.$fromLanguage.value,
        {
          keepNodeTransform: this.$keepNodeTransform.checked, 
          keepLabelOptions: this.$keepLabelOptions.checked
        });
    });
