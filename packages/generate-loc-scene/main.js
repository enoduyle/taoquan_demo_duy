...
function copyLocalizationData(folderPath, isLocFolder=false) {
  ...
  fsExtra.readdirSync(folderPath).forEach(it => {
    ...
    const isMappingFile = (it.indexOf('.png') >= 0 || it.indexOf('.jpeg') >= 0 || it.indexOf('.jpg') >= 0 
                          || it.indexOf('.plist') >= 0 || it.indexOf('.json') >=0 || it.indexOf('.prefab') >=0 || it.indexOf('.fnt') >=0);
...
function getMappingData() {
  ...
  generate_log.localizationAssets.forEach(asset => {
    languageList.concat(fromLanguage).forEach(lang => {
      ...
      else if (asset.type === '.json' || asset.type === '.prefab' || asset.type === '.fnt') {
...
async function copyRelatedRefs() {
  ...
  assets.forEach(asset => {
    if (isRelatedAsset(asset.path) && !isExcludeData(asset.path)) {
      ...
      relativePath = relativePath.replace(dataFolderPath, '');
...
async function copyNestedRefs() {
  ...
  assets.forEach(asset => {
    ...
    if (isNestedAsset(asset.path) && !isExcludeData(asset.path)) {
          ...
          if (overrideScene) {
            ...
            /*transformLock.nodes.forEach(it => {
              ...
              if (newNode && oldNode) {
                ...
                newNode._contentSize = oldNode._contentSize;
            ...
            newScene.forEach(it => {
              if (it.__type__ == 'cc.Node' && options.keepNodeTransform) {
                let oldNode = oldSceneObj[it._id];
                if (oldNode) {
                  Editor.log('Update old transform for ' + it._name);
                  it._position = oldNode._position;
                  it._scale = oldNode._scale;
                  it._contentSize = oldNode._contentSize;
                }
              }

              if (it.__type__ == 'cc.Label' && options.keepLabelOptions) {
                let oldLabel = oldSceneObj[it._id];
                if (oldLabel) {
                  Editor.log('Update old label for ' + it._name);
                  it._N$file = oldLabel._N$file;
                  it._fontSize = oldLabel._fontSize;
                  it._lineHeight = oldLabel._lineHeight;
                  it._string = oldLabel._string;
                  it._N$string = oldLabel._N$string;
                }
              }
            })
          }
